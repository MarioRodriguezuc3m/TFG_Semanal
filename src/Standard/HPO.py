...
def run_aco_with_params(alpha, beta, rho, Q, initial_pheromone, n_ants=20, iterations=100):
...
def plot_parameter_effects(df):
    ...
    os.makedirs('/app/hpo/standard', exist_ok=True)
    plt.savefig('/app/hpo/standard/parameter_effects.png')
    plt.show()
...
def random_search_hpo(n_iterations=50, n_ants=20, aco_iterations=500):
    ...
    for i in range(n_iterations):
        ...
        try:
            ...
            if (i+1) % 5 == 0:
                ...
                df.to_csv('/app/hpo/standard/aco_random_hpo_results_partial.csv', index=False)
    ...
    df_results.to_csv('/app/hpo/standard/aco_random_hpo_results.csv', index=False)
